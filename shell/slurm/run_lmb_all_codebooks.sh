#!/bin/bash
# Submit LMB experiments across all codebook sizes on Quartz (SLURM).
# Uses the train_lmb.sh script (which now includes perceptual + GAN weights).
#
# Usage:
#   bash shell/slurm/run_lmb_all_codebooks.sh
#   bash shell/slurm/run_lmb_all_codebooks.sh --dry-run
...
TRAIN_SCRIPT="$SCRIPT_DIR/train_lmb.sh"
...
echo "============================================================"
echo "  LMB experiments across codebook sizes"
...
if [[ $DRY_RUN -eq 0 ]]; then
    ...
    echo "  squeue -u \$USER"
    echo "  tail -f ${PROJECT_ROOT}/logs/slurm/lmb_cb*_256_s${SEED}_*.out"
fi
