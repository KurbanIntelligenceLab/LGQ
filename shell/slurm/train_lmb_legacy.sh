#!/bin/bash
# LMB with OLD (pre-decfix) decoder: ch = max(32, embedding_dim).
# Same perceptual/GAN losses and batch size as other methods.
#
#SBATCH --job-name=lmb_legacy
...
# Injected by submit script via --export
CODEBOOK_SIZE="${CODEBOOK_SIZE:-8192}"
DATA_ROOT="${DATA_ROOT:-/N/project/de_briujn_graph/Projects/vector-quantize/Data/imagenet}"
SEED="${SEED:-1234}"

TAG="lmb_legacy_cb${CODEBOOK_SIZE}_256_s${SEED}"
echo "=== $TAG ==="
